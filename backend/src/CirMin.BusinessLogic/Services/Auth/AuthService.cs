...
using CirMin.DataAccess.Commons.Exceptions;
...
public class AuthService : IAuthService
{
    ...
    public async Task SignUp(RegisterRequest request)
    {
        ...
        if (existedUser != null) throw new CirMinConflictException(ErrorCodes.User.UsernameAlreadyExists);

        var isEmailUnique = await _userRepository.IsEmailUniqueAsync(request.Email);
        if (!isEmailUnique) throw new CirMinConflictException(ErrorCodes.User.EmailAlreadyExists);
    ...
    public async Task<AuthResponse> SignIn(LoginRequest request)
    {
        ...
        if (user == null) throw new CirMinUnauthorizedException(ErrorCodes.Auth.InvalidCredentials);

        var isMatchPassword = _passwordHasher.VerifyHashedPassword(user.HashedPassword, request.Password);

        if (!isMatchPassword) throw new CirMinUnauthorizedException(ErrorCodes.Auth.InvalidCredentials);
    ...
    public async Task<AuthResponse> RefreshToken(string oldToken, CancellationToken cancellationToken)
    {
        ...
        if (token == null) throw new CirMinUnauthorizedException(ErrorCodes.Auth.InvalidSession);

        //2. Check revoke and expiredTime
        if (token.IsRevoked || token.ExpiresAt < DateTimeOffset.UtcNow)
            throw new CirMinUnauthorizedException(ErrorCodes.Auth.SessionExpired);
